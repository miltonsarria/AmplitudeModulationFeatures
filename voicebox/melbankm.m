...
if ~any(w=='H')
    switch wr
        ...
        case 'A'
            mflh=frq2exp(mflh);       % convert frequency limits into exp
        case 'W'
            mflh=frq2wss(mflh);       % convert frequency limits into wss
        otherwise
...
switch wr
    ...
    case 'A'
        blim=exp2frq(mflh(1)+[0 1 p p+1]*melinc)*n/fs;
    case 'W'
        blim=wss2frq(mflh(1)+[0 1 p p+1]*melinc)*n/fs;
    otherwise
...
switch wr
    ...
    case 'A'
        pf=(frq2exp((b1:b4)*fs/n)-mflh(1))/melinc; 
    case 'W'
        pf=(frq2wss((b1:b4)*fs/n)-mflh(1))/melinc; 
    otherwise
